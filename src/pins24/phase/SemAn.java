...
public class SemAn {
	...
	private static class NameResolver {
		 ...
		 */
		private class ResolverVisitor implements AST.FullVisitor<Object, ResolverVisitor.Pass> { //gre po celemu drevesu in zna sam it okrog
																								// par metod je redefinirano
			...
			public Object visit(final AST.VarExpr varExpr, final Pass pass) {
				...
				attrAST.attrDef.put(varExpr, def); //od uporabe imena do definicije imena
				return null;
			}
			...
			public Object visit(final AST.CallExpr callExpr, final Pass pass) {
				...
				callExpr.args.accept(this, null); // obdelaj se parametre
				return null;
			}
	...
	private static class LValResolver {
		...
		private class ResolverVisitor implements AST.FullVisitor<Object, Object> {
			...
			public Object visit(final AST.UnExpr unExpr, final Object arg) {
				...
				// 2. pravilo jezika
				// Izraz kome je je krajnji operator ()^ je lval
				...
				// nemoguće je dobiti adresu konstante (^3)
				if (unExpr.oper == AST.UnExpr.Oper.MEMADDR && !(unExpr.expr instanceof AST.VarExpr)) {
					throw new Report.Error(attrAST.attrLoc.get(unExpr), "Only variables can be referenced.");
				}
			...
			@Override
			public Object visit(final AST.AssignStmt assignStmt, final Object a) {
				switch(assignStmt.dstExpr) {
					...
					case final AST.UnExpr unExpr:
						if (unExpr.oper == AST.UnExpr.Oper.VALUEAT) {
							attrAST.attrLVal.put(unExpr, true);
							unExpr.accept(this, a);
							break;
						} else {
							throw new Report.Error(attrAST.attrLoc.get(assignStmt.dstExpr),
									"Cannot assign value to non variable");
						}
					default:
						throw new Report.Error(attrAST.attrLoc.get(assignStmt.dstExpr),
												"Cannot assign value to non variable");
				}
				assignStmt.srcExpr.accept(this, a);
				return null;
			}
